function [theInput, theOutput, maxmin] = Quad_12_nln_Datagenerator_ss(lb, ub, nn, timestep, normalization, num_traj , horizon, avg_m, cov_m, avg_s, cov_s) 
...
parfor j = 1:num_traj
    ...
    for i=1:horizon
        init_a = initial;
        a_control=NN(nn, init_a);
        noise = mvnrnd(avg_s, cov_s, 1);
        [~,in_out] =  ode45(@(t,x)Quad_12(t,x,a_control, noise),[0 timestep],initial');
        in_out=in_out(end,:)';
        index = n*i;
        nu = mvnrnd(avg_m , cov_m , 1 )';
