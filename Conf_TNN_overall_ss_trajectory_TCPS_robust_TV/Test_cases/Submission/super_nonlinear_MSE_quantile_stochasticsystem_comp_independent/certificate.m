...
load('Main_network_super_nonlinear_stochasticsystem.mat')
...
Output_Data = gather(Output_Data);
Try_in = 10000;
decision = zeros(1, Try_in);
experi_test = Output_Data( :  , randperm(300000 , Try_in) );

n=2;

parfor j=1: Try_in

    Decision = check_contains_ayahast(Star_setss , experi_test(3:end, j));

    if Decision == 1
        decision(j) = 1;
    else
        disp('ayahast mentioned No')
        decision(j) = check_contains_agenist(Star_setss , experi_test(3:end, j) , n);
        if decision(j)==1
